data "archive_file" "inline_scorer" {
  type        = "zip"
  source_file = "${path.module}/lambda/inline_scorer.py"
  output_path = "${path.module}/lambda/inline_scorer.zip"
}
...
resource "aws_iam_role_policy" "lambda_scorer" {
  name   = "InlineScorerAccess"
  role   = aws_iam_role.lambda_scorer.id
  policy = data.aws_iam_policy_document.lambda_permissions.json
}

resource "aws_lambda_function" "inline_scorer" {
  function_name    = "${var.agent_name}-inline-scorer"
  role             = aws_iam_role.lambda_scorer.arn
  handler          = "inline_scorer.handler"
  runtime          = "python3.12"
  timeout          = 120
  filename         = data.archive_file.inline_scorer.output_path
  source_code_hash = data.archive_file.inline_scorer.output_base64sha256
...
resource "aws_lambda_function_url" "inline_scorer" {
  function_name      = aws_lambda_function.inline_scorer.function_name
